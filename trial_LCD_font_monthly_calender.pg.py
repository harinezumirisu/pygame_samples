...
for n in range(z):  # nはcol2行分で各日にちを表し、各月の日数分であるｚ回繰り返して日付表示する
    ...
    lcdx.update_col(col=((3 * n + 1) + (wd + 1) % 7 * 3)  % 21, code=(n + 1) // 10) # カレンダー各日10の位
    lcdx.update_col(col=((3 * n + 2) + (wd + 1) % 7 * 3 ) % 21, code=((n + 1) % 10)) # カレンダー各日1の位

o = dt_now.day -1  # 今日の日付の色を変更する
xo =3 + ((3 * o + 2) + (wd + 1) % 7 * 3)// 21
...
lcdxo.update_col(col=((3 * o + 1) + (wd + 1) % 7 * 3)  % 21, code=(o + 1) // 10) # カレンダー各日10の位
lcdxo.update_col(col=((3 * o + 2) + (wd + 1) % 7 * 3 ) % 21, code=((o + 1) % 10)) # カレンダー各日1の位
